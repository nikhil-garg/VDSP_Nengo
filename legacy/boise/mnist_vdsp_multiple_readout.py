...
def evaluate_mnist_multiple(args):
    ...
    for num in range(input_nbr):
        ...
        true_label = labels[(num*int(presentation_time/args.dt))+1]
        X_train.append(num_spikes)
        Y_train.append(true_label)

    clf_lda = SVC(gamma='auto', probability=True)
    ...
    del weights, sim.data, labels, output_spikes
